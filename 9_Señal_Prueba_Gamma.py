'''
OBJETIVO => probar el algoritmo de detección de bordes con Distribución Gamma
...
def calcular_parametros_gamma(ventana):
    """Calcula los parámetros de la distribución Gamma para la ventana."""
    shape, loc, scale = gamma.fit(ventana,floc=0)  # Ajusta la distribución Gamma a los datos
    #print(f"Parámetros de Gamma para ventana: alfa = {shape}, gamma = {scale}")  # Imprimir parámetros calculados
    return shape, scale  # Retorna los parámetros alfa (shape) y beta (scale)
...
def calcular_diferencia_alfa(ventanas):
    ...
    for i in range(len(ventanas)-1):
        alfa1, _ = calcular_parametros_gamma(ventanas[i])
        alfa2, _ = calcular_parametros_gamma(ventanas[i+1])
...
def negativo_log_verosimilitud_gamma(ventana, alpha, gamma_scale, epsilon=1e-10):
    """Calcula el negativo de la log-verosimilitud para una ventana de datos
    usando los parámetros alpha y gamma (scale) de la distribución Gamma."""

    log_likelihood = 0
    for z in ventana:
        # Evaluamos la función de densidad de probabilidad Gamma para el valor z
        pdf_value = gamma.pdf(z, a=alpha, scale=gamma_scale)
...
def encontrar_punto_cambio_verosimilitud(inicio_ventana1,ventana1, ventana2):
    ...
    for punto_cambio in range(10, n_total-10):
        ...
        if len(parte1) > 0 and len(parte2) > 0:
            alpha1, gamma1 = calcular_parametros_gamma(parte1)
            alpha2, gamma2 = calcular_parametros_gamma(parte2)

            if alpha1 is not None and gamma1 is not None and alpha2 is not None and gamma2 is not None:
                log_vero1 = -negativo_log_verosimilitud_gamma(parte1, alpha1, gamma1)
                log_vero2 = -negativo_log_verosimilitud_gamma(parte2, alpha2, gamma2)
...
for i, ventana in enumerate(ventanas):
       alpha, gamma_scale = calcular_parametros_gamma(ventana)
